...
#
# 눈으로 지킨 규칙은 남지 않는다. 폼에 넣는 문서는 check-form-text.py가 막고 있어 엠대시가
...
#
# **엔진은 상류(pqcota)에 있고, 이 리포에는 설정만 있다**(tools/checkprose/의 rules·notices·
# overlap·files·baseline). 판을 고정해 돌리고 최신을 받지 않는다. 올릴 때는 여기 판을 바꾸고
# 관문을 다시 돌려 기준선이 그대로인지 본다. 계약 의존은 go.mod가 정하고 이 판과 무관하다.
...
#
# 그 두 장은 문구를 두 벌 적어 두고 CSS로 한쪽을 숨긴다. **한쪽 말로 볼 때는 멀쩡해 보이는
...
# 그 HTML을 **어떻게 읽는가**에서 생기므로 같은 규칙으로 읽는 파서라야 보인다.
check-lang:
	@go run ./tools/checklang

# 케이스 관문: docs/testcases.md의 번호와 실제 테스트를 맞댄다.
#
...
# 함께 움직이는데, 빌드도 테스트도 그것을 보지 않는다. 며칠 쌓이면 그 다음 diff에서 진짜
# 바뀐 줄이 정렬 줄에 묻힌다.
check-fmt:
	@out=$$(gofmt -l .); \
	if [ -n "$$out" ]; then \
...
# ── 화면 템플릿 ────────────────────────────────────────────────────────────
#
# templ이 .templ을 *_templ.go로 옮긴다. **생성물을 리포에 함께 둔다.** 그래서
...
# 무엇을 보고 있는지가 어긋난다. all에 넣지 않은 것은 이것만 망을 타기 때문이다.
verify-generated:
	@go run $(TEMPL) generate
