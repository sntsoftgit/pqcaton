...
| **[IC-K15](../tools/checkprose/main_test.go) ✅** | 함께 나가는 `rules.tsv`의 「띄운 조사」 규칙에 실제 문장 | `에만`·`나`·`까지만`·`로만`·`뿐`처럼 뒤늦게 더한 조사는 잡고, 「할 뿐」·「쓰다 만」·HTML 태그 뒤의 「뿐」은 잡지 않는다. 규칙을 테스트 안에 따로 적으면 파일이 바뀌어도 통과하므로 나가는 파일을 읽는다 |
