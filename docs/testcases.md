...
| **[IC-P14](../pkg/inventory/review/axes_test.go) ✅** | 같은 자산이 자동통과에서 리뷰 항목으로(확신 하락) 옮겨 가며 지문도 바뀜 · 아무것도 안 바뀜 · 자동통과의 근거만 바뀜 · 옛 식별자 중 하나만 새 세션에 있음 | 계획 선택값(포함·종류·수준·목표·되돌림 메모)이 **따라온다.** 근거가 바뀌면 서명이 지워지고, 그대로면 남는다. **자동통과의 근거만 바뀌어도** 서명이 무효다. 옛 식별자는 같은 ID 가 있으면 치환된 셈이고 없는 것만 남는다 | `sameBasis` 는 자리(index)가 아니라 **ID** 로 맞춘다. 항목이 컬렉션을 옮기면 길이도 자리도 어긋난다. `All` 은 복사본이라 쓰는 자리는 `update` 로 실제 컬렉션을 찾는다 |
...
| **[IC-P16](../pkg/inventory/review/axes_test.go) ✅** | v2 세션의 빈 되돌림 메모 · v3 세션의 빈 되돌림 메모 · 적은 되돌림 메모 | v2 는 결론으로 채워지고, **v3 은 빈 값 그대로**, 적은 것은 그대로 나간다 | `rollback_note` 는 계획 축의 값이다. 결론은 「어떻게 판정했나」이고 이것은 「어떻게 되돌리나」다. v3 사용자가 일부러 비운 자리에 결론이 다시 들어가면 사람의 선택을 덮는 것이다 |
| **[IC-P17](../pkg/inventory/review/axes_test.go) ✅** | 같은 ID 가 리뷰 항목과 자동통과 양쪽에 있음 | **확정이 구조 오류로 중단한다**(판정 미완이 아니다) | 한 자산은 한 컬렉션에만 있다. 양쪽에 있으면 대조의 결함이고, 그대로 두면 `Carry` 가 한쪽에만 쓰고 다른 쪽이 덮는다 |
...
| [IC-U31](../inventory/cmd/pqcaton-ui/main_test.go) ✅ | 옛 화면의 이동 링크 | 새 화면이 없다. 절차 화면과 나란히 두면 어느 쪽이 진짜인지 헷갈린다. **IC-U25 와 같은 선이다** |
...
| **[IC-U37](../pkg/inventory/ui/ui_test.go) ✅** | 스코프의 두 화면 | 같은 폼을 쓴다(`/scope/finalize`). 확정 경로가 두 벌이 되면 **승인한 것과 확정된 것이 달라지는 날**이 온다. IC-U30 과 같은 선이다 |
...
| TC | Given → When | Then |
|---|---|---|
| **[IC-B1](../tools/checklang/main_test.go) ✅** | `</span>` 이 빠져 한국어가 영어를 품은 자리 | 잡는다. **2026-08-27 에 실제로 난 고장의 모양이다.** 한국어로 보면 멀쩡해서 사람 눈으로는 걸러지지 않던 자리다 |
...
| **[IC-B7](../tools/checklang/main_test.go) ✅** | 「정의」처럼 짧고 흔한 낱말이 앞쪽에도 있는 원본 | 앞쪽이 아니라 **실제 자리의 줄**을 짚는다. 품은 글자를 원본에서 되찾는 첫 판은 437 을 357 로, 438 을 242 로 짚었다. 그래서 토크나이저로 태그가 나온 줄을 센다 |
...
> 세는 방식은 아홉 자리를 모두 통과시킵니다. 고장이 브라우저가 그 HTML 을 **어떻게 읽는가**에서
