...
**pqcota의 여정을 그대로 두고, 이 리포가 들어오는 자리만 갈라 표시했습니다.** 흐린 상자는
pqcota가 하는 일이고 번호는 [그쪽 여정](https://github.com/randyinthedev-hash/pqcota/blob/main/docs/journey.md)의
것입니다. 굵은 상자가 이 리포이고 번호는 이 문서의 절입니다.

```mermaid
flowchart TB
    A["0 · 도입 판단"] --> B

    subgraph infra["자기 인프라 — 전부 한 상자 안"]
      direction TB

      B["1 · 준비<br/>pqcota 1"] --> C["접근 준비<br/>pqcota 2 · 여러 노드일 때만"]
      C --> D["3 · 관측<br/>pqcota 3"]
      D --> E["적재<br/>pqcota 4"]
      W["위임 수신<br/>pqcota 3′ · CI가 낸 CycloneDX"] --> E
      E --> F["조회<br/>pqcota 5"]

      subgraph us["pqcaton — 비어 있던 자리를 채운다"]
        direction TB
        P["2 · 선언 반입<br/>CMDB"] --> Q["4 · 대조<br/>3-상태 · confidence"]
        Q --> R["리뷰 큐"]
        R --> S["5 · 리뷰 · 확정<br/>승인 서명"]
      end

      E --> Q
      S --> G["확정 계획<br/>pqcota 6 · FinalizedPlan"]
      G --> H["6 · 생성 · 적용 · 되돌림<br/>pqcota 7·8"]
      H --> J["7 · 계속<br/>pqcota 9"]
    end

    F -.->|"pqcaton 없이 — 손으로 쓴 계획"| G
    D -.->|"한 노드를 그 자리에서"| F
    J -.->|"재관측 · 변화 diff"| D
    J -.->|"근거가 바뀐 판정만"| R
```

**이 그림의 요점은 「확정 계획」 상자입니다.** pqcota는 그것을 **입력으로만** 받습니다 —
*무엇을 언제 바꿀지는 그 도구가 정하지 않고*, 손으로 쓴 계획으로도 끝까지 돕니다(점선).
**그 자리를 채우는 것이 이 리포입니다.**

| 그림에서 읽을 것 | |
|---|---|
| **왼쪽 위 점선** | pqcaton이 없어도 파이프라인은 끊기지 않습니다. 계획을 사람이 씁니다 |
| **`적재 → 대조`** | 이 리포는 pqcota가 쌓은 것을 **읽기만** 합니다. 관측을 다시 하지 않습니다 |
| **`확정 → 확정 계획`** | 넘어가는 것은 `FinalizedPlan` 하나입니다. 그것도 **계약이 정한 형식**입니다 |
| **아래 점선 둘** | 재관측은 pqcota로, 델타 리뷰는 이 리포의 리뷰 큐로 — **되돌아오는 길이 둘로 갈립니다** |
| **한 상자 안** | 클라우드가 없습니다. 밖으로 나가는 것이 하나도 없습니다 |
...
### 그림의 나머지 상자들도 여기 있습니다

한눈에 그림에서 이 리포가 아닌 상자 넷은 전부 pqcota 여정의 것이고, **이 문서는 그것을 다시
설명하지 않습니다.** 어디로 가서 읽을지만 적어 둡니다.

| 상자 | 무엇 | 어디에 |
|---|---|---|
| **접근 준비**(pqcota 2) | 접속 정보 CSV 한 벌에서 Ansible 인벤토리와 노드 표가 갈려 나옵니다. **여러 노드를 훑을 때만** 필요합니다 | pqcota 여정 §2 |
| **위임 수신**(pqcota 3′) | CI가 낸 CycloneDX를 관측 대신 받습니다 — 빌드 시점의 자산은 런타임에서 안 보이는 것이 있습니다 | pqcota 여정 §3′ |
| **적재**(pqcota 4) | `pqcota-ingest`가 결과를 히스토리에 쌓습니다. **이 리포가 읽는 것이 이 저장소입니다** | pqcota 여정 §4 |
| **조회**(pqcota 5) | `pqcota-inventory`가 최신·이력·diff를 냅니다. 사람이 눈으로 보는 자리이고 **대조와는 다른 일**입니다 | pqcota 여정 §5 |

**한 노드만 볼 거면 접근 준비와 적재를 건너뜁니다**(그림의 `관측 → 조회` 점선). 그때는 이 리포도
