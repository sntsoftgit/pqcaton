...
리뷰 세션과 스코프 세션은 **입력 파일에서 파생됩니다**. 입력 파일이 있으면 명령도 화면도
그것을 만들 수 있습니다. 입력 파일이 이미 있는데도 명령을 한 번 돌려야 화면이 열리는 것은,
...
> 적이 없습니다. 그대로 두면 방금 나타난 UNDECLARED나 방금 넣은 `exclude`가 **누가 승인한 적
...
문서를 만드는 비용이 편집 화면을 만드는 비용보다 큽니다. **그래서 설명을 「규칙을 적는 법」 도움말로 접어 두고,
표를 화면에서 고치게 했습니다.**
...
계층 **구성**(어느 파일이 어느 순서로 겹치는지)은 조직 구조라 명령줄에서 정합니다. `-layers`의
순서가 곧 상속 순서입니다.
...
조직 필드가 없어 엔진이 스스로 알아낼 방법이 없으므로, 조직은 부르는 쪽이 단언하고 그 단언이
어긋나는 순간을 엔진이 잡습니다.

섞인 입력을 그대로 두면 **오류가 아니라 그럴듯한 결과**가 나오는 것이 이 검사가 필요한 이유입니다.
조직이 동일성의 일부라 식별자가 맞지 않고, 같은 자산이 UNDECLARED와 UNOBSERVED 한 쌍으로 구분되어
리뷰 큐에는 새로 나타난 UNDECLARED로 오릅니다. 판정 원장의 대상 id에는 조직을 넣지 않습니다.
...
draft ──▶ in-review ──▶ finalized
                         └ 전 필수항목 판정 + 판정자 서명 (전제)
```
- **finalized 전에는 계획이 만들어지지 않습니다**(§5. 반드시 거쳐야 하는 관문). 링/도메인 단위 부분 확정은 허용합니다.
- **이 리포의 `finalized`는 계약의 `FINALIZED`가 아닙니다.** 여기서 finalized는 **판정 세션이 닫혔다**는 뜻입니다. 계약의 `FINALIZED`는 **실행 승인까지 끝났다**는 뜻이고, 그것은 상류의 승인자가 자기 키로 서명해야 생깁니다(`pqcota-approve`). 판정과 실행 승인은 다른 단계라, 이 리포는 판정이 끝난 계획을 계약의 **`IN_REVIEW`**로 내보내고 승인 칸(`approval_signatures`)과 확정 시각(`finalized_at`)을 **비워 둡니다.** 둘 다 실행 승인의 자리입니다.
- **판정자 서명은 검증되지 않습니다.** 검토자가 손으로 적는 자유 문자열이라 「누가 판정했나」가 아니라 **「누가 판정했다고 기록됐나」**까지만 뜻합니다. 그 기록의 무게는 판정 원장이 append-only라는 데서 옵니다. 전에는 이 문자열을 계약의 승인 칸에 넣었는데, 그러면 아무것도 증명하지 않는 값이 **상류 구조 관문의 「승인 항목 있음」을 충족하는 모양**이 됩니다. 이제 넣지 않습니다.

| | 무엇에 답하나 | 무엇으로 서나 |
|---|---|---|
| 이 리포의 판정자 서명 | 누가 판정했다고 기록됐나 | 검증되지 않는다. 원장이 append-only라는 것이 근거다 |
| 상류의 승인 서명 | 누가 실행을 승인했나 | 컨트롤러에 등록된 승인자의 공개키 |

- **세션 id가 계획과 원장을 잇습니다.** 세션을 열 때 UUID를 만들어 세션 파일에 저장하고, 다시 열어도 그대로입니다(새 세션이면 내용이 같아도 다릅니다). 계약으로 나가는 계획의 id가 `pqcaton:<스코프>:<세션 id>`이고, 판정 원장 행도 같은 값을 들고 가서 `BySessionID`로 되짚습니다. 「어느 계획 사건인가」를 답하는 값이라 내용 해시가 아닙니다. 같은 내용이 두 번 승인·실행될 수 있고, 그때 감사 기록이 겹치면 안 됩니다. 세션 id가 없는 세션(옛 빌드가 연 것)은 확정하지 않고 다시 열라고 안내합니다. 여기서 새로 찍으면 계획은 세션을 가리키는데 원장에는 그 세션이 없게 됩니다.
- **리뷰 단위(§3.4, 하이브리드)**: 정책 단위가 기본입니다(버전×링크모드·JDK×provider 카탈로그가 리뷰 대상 정책 템플릿이므로 동종 자산을 일괄로 봅니다). 개별 격리는 예외이며, 정책 예외·고위험·UNDECLARED 엣지만 엣지 단위로 봅니다.
...
**조직 타입은 pqcota의 `pkg/org`를 그대로 씁니다.** 규칙은 그대로입니다. **빈 조직으로는
저장소를 열 수 없습니다.**

전에는 이 리포가 같은 이름의 패키지를 따로 두고 있었는데, pqcota v0.2.0이 인벤토리에 조직
...
질의마다 `org`를 다는 것은 **우리가 안 틀린다는 전제**에 기대고 있습니다. 여러 조직이 한
