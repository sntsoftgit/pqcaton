...
| [`pqcaton-ui`](../inventory/cmd/pqcaton-ui) | 위 전부를 **화면에서** 다룹니다 — 선언과 스코프 규칙을 고치고, 대조를 보고, 판정·확정하고, 인벤토리를 찾아봅니다. **세션도 화면이 엽니다** | 선언 · 관측 결과 · 계층 CSV · 판정 원장 | 같은 파일에 씁니다 — **같은 게이트** |
...
> 같은 게이트로 지납니다 — 재료만 주면 세션까지 화면이 엽니다(아래 「세션은 재료에서
...
draft ──▶ in-review ──▶ finalized
                         └ 전 필수항목 판정 + 승인 서명 (전제)
```
- **finalized 전에는 프로비저닝을 실행할 수 없습니다**(§3.7 — 우회할 수 없는 게이트). 링/도메인 단위 부분 확정은 허용합니다.
