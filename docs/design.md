...
인벤토리는 **한 조직의 암호 자산 지도**다. 다른 조직의 것이 섞이면 그 자체로 사고다 —
계열사 분리든 여러 고객을 함께 다루든, 경계는 **데이터에 박혀 있어야 하지 질의를 쓰는
사람의 기억에 있으면 안 된다.**

### 2.1 격리를 끌 수 없게 만든다

`pkg/org`가 식별자 하나와 규칙 하나를 갖는다 — **빈 조직으로는 저장소를 열 수 없다.**

저장소 핸들이 조직에 묶인다. `NewPgJudgmentStore(ctx, dsn, org)`가 돌려주는 핸들의 모든
질의에 `WHERE org = $1`이 이미 붙어 있고, **그것을 빼는 방법이 없다.** 질의마다 조건을
기억할 일이 없으니 잊을 일도 없다.

| | |
|---|---|
| 인덱스 | `(org, subject, seq)` — 선두가 org라 모든 조회가 조직으로 먼저 걸린다 |
| 인메모리 저장소 | Pg판과 **같은 규칙**. 테스트가 격리 없는 경로를 타면 실제와 어긋난다 |
| 식별자 모양 | 소문자·숫자·하이픈. 대소문자를 섞으면 `Acme`와 `acme`가 다른 조직이 되어 데이터가 조용히 갈린다 |

### 2.2 무엇을 아직 하지 않았나

- **인증·권한** — 누가 어느 조직에 속하는지는 이 계층이 정하지 않는다. 호출자가 이미
  확인한 조직을 받는다.
- **대조 엔진** — `reconcile`은 아직 조직을 받지 않는다. 입력 스냅샷이 한 조직의 것이라는
  전제로 돈다. 여러 조직을 한 프로세스에서 다루게 되면 그때 같은 방식으로 묶는다.
- **행 수준 보안(RLS)** — Postgres RLS로 한 겹 더 걸 수 있다. 핸들 격리가 뚫려도 DB가
  막는 구조인데, 운영 복잡도가 올라가므로 실제 멀티테넌트 운영이 정해진 뒤에 넣는다.
