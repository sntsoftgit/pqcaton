...
이 리포의 명령들이 **무엇을 입력으로 받고 무엇을 출력하는지**, 어떤 차례로 실행되는지
적었습니다. 각 단계의 설계 근거는 아래 §1부터에 있습니다.
...
> 같은 관문으로 지납니다. 입력 파일만 주면 세션까지 화면이 만듭니다(아래 「세션은 입력
> 파일에서 나옵니다」). **명령의 출력은 영어이고, 화면은 한국어와 English 중에 고릅니다**
