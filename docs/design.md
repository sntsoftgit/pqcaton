...
리뷰 큐가 그것을 UNDECLARED 발견으로 올립니다. 판정 원장의 대상 id에는 조직을 넣지 않습니다 —
...
- **리뷰 granularity(§3.4, 하이브리드)**: 정책 단위가 기본입니다(버전×링크모드·JDK×provider 카탈로그가 리뷰 대상 정책 템플릿 → 동종 자산 일괄). 개별 격리는 예외이며, 정책 예외·고위험·UNDECLARED 엣지만 엣지 단위로 봅니다.
