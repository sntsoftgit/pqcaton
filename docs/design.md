...
### 세션은 명령이 만들고, 화면은 채웁니다

`pqcaton-ui` 는 **세션을 열지 못합니다.** 세션을 여는 일은 관측·대조가 필요하고, 화면은 그
결과를 채우고 확정하는 자리입니다 — 그래서 화면을 열기 전에 다룰 파일이 있어야 합니다.

| 화면 탭 | 그 파일을 누가 만드나 | 수명 |
|---|---|---|
| ① 선언 | 사람이 처음 씁니다(데모는 `declare.py`). **화면이 그대로 편집합니다** | 계속 씁니다 |
| ② 스코프 | **`pqcaton-scope open`** — `-base` 와 견줘 **바뀐 규칙만** 담깁니다 | `close` 로 정책이 나오면 끝 |
| ③ 대조 | 만들 것이 없습니다 — 결과 디렉터리를 읽습니다 | — |
| ④ 리뷰 큐 | **`pqcaton-decide open`** | `close` 로 계획이 나오면 끝 |

세션 둘은 **한 번 쓰고 버리는 것**입니다. 다음에 정책이나 판정을 또 다룰 때 `open` 으로 새로
만듭니다 — 남는 것은 확정된 산출물(`asset-scope.csv`·`plan.json`)과 판정 원장입니다.
