...
- **대조 엔진** — `reconcile`은 아직 조직을 받지 않습니다. 입력 스냅샷이 한 조직의 것이라는
  전제로 돕니다. **그 전제가 곧 깨집니다** — 호스팅에서 무료 고객 전체가 한 인스턴스를
  함께 쓰기로 했고(2026-08-13), 그러면 여러 조직을 한 프로세스에서 다루게 됩니다. 판정
  저장소는 이미 조직에 묶였으니 **대조 엔진만 남았습니다.**
- **행 수준 보안(RLS)** — Postgres RLS로 한 겹 더 걸 수 있습니다. 핸들 격리가 뚫려도 DB가
  막는 구조입니다. *"실제 멀티테넌트 운영이 정해진 뒤에"* 로 미뤄 두었는데 **그것도
  정해졌습니다** — 무료 인스턴스가 바로 그 자리입니다. 운영 복잡도와 견주어 다시 봅니다.
