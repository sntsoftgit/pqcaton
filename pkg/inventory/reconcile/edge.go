...
func ReconcileEdges(declared []EdgeKey, observed []*discoveryv1.ObservedEdge, scope map[string]bool, gapLayers []string) []ReconciledEdge {
	...
	for _, oe := range observed {
		key, offScope := observedEdgeKey(oe, scope)
		if seen[key] {
			continue
		}
		seen[key] = true
...
func observedEdgeKey(oe *discoveryv1.ObservedEdge, scope map[string]bool) (EdgeKey, bool) {
	dst := oe.GetDstNodeId()
	offScope := false
	if dst == "" {
