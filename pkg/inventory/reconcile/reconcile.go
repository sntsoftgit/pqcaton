...
// ManagedState — 관리 축. **대조 축(State)과 다른 것을 잰다.** State 는 선언과 관측의
...
const (
	...
	// NotEvaluated — 관측이 없어 정책을 걸 대상이 없었다. MANAGED 로 적으면 하지 않은 판정을
	// 기록하는 것이고, EXCLUDED 로 적으면 정책이 뺀 것과 구별되지 않는다.
...
type Observed struct {
	...
	// 이 값으로 가른다.
	Fingerprint string
	// appKeys — 이 finding 에 걸린 앱 열쇠 전부. 제외 근거의 경고문이 쓴다. 관리 근거에는
	// 필요 없어 내보내지 않는다(소문자).
	appKeys []string
...
//
// 스냅샷 위치가 없다: 이 finding 은 중앙 이력의 스냅샷에 들어가지 않았으므로 가리킬 위치가
// 없다. Source 와 형을 따로 두는 것은 계약 변환 함수가 이 값을 받지 못하게 하려는 것이다 -
...
type ExcludedSource struct {
	...
	// AppKeys — 이 finding 에 걸린 앱 열쇠 **전부**. 계약이 repeated 이고, 공유 .so 하나를
	// 여러 앱이 로드하면 여럿이다. 하나만 적으면 사람이 「이 앱만 빠졌다」고 읽는데 실제로는
	// 목록 전부가 함께 빠졌다. 경고문이 쓴다 - 사람이 정책 파일에서 이 열쇠들에 걸리는 줄을
	// 찾는다. **규칙 이름은 담지 않는다**: 상류 Managed 는 bool 만 돌려주므로 알 수 없다.
	AppKeys []string
}

// Excluded — 정책이 뺀 관측 하나. Observed 와 짝이고, 대조에 들어가 관리 축을 정한다.
...
func reconcileAssets(declared []AssetKey, observed []Observed, excluded []Excluded, gapLayers []string) []Reconciled {
	...
	for _, k := range order {
		...
		if len(srcs) == 0 {
			// 제외 전용. 대표 근거를 비운다 - 최상위 FindingID 는 계약의 호환용 finding_id 로
	...
	// 선언만 있고 관측 안 됨 → UNOBSERVED. 커버리지 갭이면 재수집 후보. 정책을 걸 finding 이
