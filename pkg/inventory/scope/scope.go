...
//
// **규칙의 정의와 집행은 상류가 한다.** pqcota의 `kscope.AssetPolicy`가 규칙 형식이고,
// `pqcota-ingest -scope-assets`가 적재 전에 집행한다. 이 패키지는 그것을 재구현하지 않는다 —
// 상류 타입을 그대로 쓰고, 그 위에 **조직에서만 필요한 것**을 얹는다:
//
//   - 계층 상속 — 조직 → 환경 → 노드군을 겹친다
//   - 변경 리뷰 — 무엇이 늘고 줄었는지 골라 리뷰-확정 상태기계에 태운다
//   - 배포 — 확정된 정책을 상류 집행기가 읽는 CSV로 낸다
//
...
import (
	...
	// 상류의 스코프 커널. **우리 패키지와 이름이 같아 별칭을 둔다** — 규칙 형식과 판정은
...
//
// **상속 규칙을 따로 만들지 않는다.** 상류가 이미 "매치되는 마지막 규칙이 결정"이라고
// 정해 두었으므로, 상위를 앞에 하위를 뒤에 두면 그대로 하위 우선이 된다 — 판정 규칙이
// 세상에 하나만 존재한다. 우리가 잠금을 따로 두면 내려보낸 CSV를 상류가 집행한 결과와
// 우리 화면이 갈라진다.
//
...
type Change struct {
	...
	//
	// **exclude 추가가 그것이다.** 무언가를 인벤토리에서 빼는 결정이라 사고 뒤에 근거를
	// 대야 한다. 규칙이 사라지는 쪽(제외를 거두는 것)은 인벤토리가 넓어지는 방향이라
	// 같은 무게가 아니다.
	Audited bool
}
...
// 다른 규칙으로 보여서는 안 된다. 빈 칸은 상류와 같이 `*`로 읽는다.
...
// WriteCSV — 확정된 정책을 **상류 집행기가 읽는 형식 그대로** 낸다.
//
// `pqcota-ingest -scope-assets` 의 입력이다. 우리 형식을 따로 만들면 「거버넌스가 확정한
// 정책을 상류가 집행한다」가 코드로는 거짓이 된다.
