...
//
// **상류의 ErrNotFinalized와 다른 것이다.** 저쪽은 실행 승인이 없다는 뜻이고, 이쪽은 판정이
...
type JudgedPlan struct {
	...
	// 문자열이라 「누가 판정했나」가 아니라 「누가 판정했다고 기록됐나」까지만 나타낸다. 계약의
	// approval_signatures에는 넣지 않는다 — 그 칸은 실행 승인의 자리다.
	ReviewerSig string
}
...
//
// Deploy를 허용하는 관문이 아니다. 전에는 AcceptForDeploy였고 「Inventory→Deploy가 반드시
// 거쳐야 하는 관문 · 실행 허용」이라고 적혀 있었는데, 이 리포는 실행을 허용하지 않는다. 실행
// 허용은 상류의 Executable과 승인 검증이 한다. 여기서 보는 것은 **판정 세션이 닫혔다는 표시가
// 있는가** 하나다.
...
func RouteProvider(runtime string, fipsRequired bool) string {
	...
	// **CNG는 provider를 갈아 끼우는 조치가 아니다.** 빠뜨린 것이 아니라 여기 답이
	// 없는 런타임이라, 갈래를 두어 그 사실을 적어 둔다.
	//
	// 갈아 끼울 대상이 없다 — 관측된 provider가 전부 Microsoft 이름이고, 알고리즘을
	// 실제로 서비스하는 것은 그중 하나다(pqcota v0.6.0·v0.6.1 실측). PQC 가능 여부는
	// **Windows 빌드**가 정하고, FIPS는 provider 설치가 아니라 **OS 정책**이다.
	//
	// 게다가 CNG의 `fips_validation`은 **알 수 없다**(알고리즘 열거로는 판별되지
