...
#
# v0.18.0 전에는 여기서 드러난 것이 있었다: **자동통과(CONFIRMED·고신뢰)한 자산을 조치로 가져가는 길이 없었다.** 자동통과가
# 리뷰 큐에 항목으로 들어가지 않아 계획 칸을 들지 못했다. 자동통과는 「사람이 볼 필요가 없다」는
# 뜻이지 「바꿀 필요가 없다」가 아니라, 지금은 계획 칸을 들고 위 판정 단계에서 계획에 넣는다.
