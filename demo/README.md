...
전부 빼, 정책이 관리 대상으로 남긴 관측이 모두 UNDECLARED가 됩니다. 그래야 **이 리포가 만든 계획으로**
승인 → 생성 → 레코드 → 실제 스냅샷 id까지 한 바퀴가 돕니다. **데모 서사가 아니라 검증용입니다** —
없는 자산을 지어내지 않고 선언을 비울 뿐이며, 그것은 선언을 아직 적지 않은 조직에서 실제로 있을 수
