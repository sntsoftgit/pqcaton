...
docker run -d --restart=always \
  ...
  -e PQCATON_EXPOSE_ADDR=unmanaged \
...
| `PQCATON_EXPOSE_ADDR` | 기본 `unmanaged` — **미등재 상대의 주소만** 올립니다. 등재된 노드의 주소는 안 올립니다. `off`·`on`도 고객이 고릅니다([§6.3.1](design.md)) |
...
| **주소 노출** | 기본 `unmanaged` — **미등재 상대의 주소만** 올라옵니다. 등재된 노드는 `node_id`로 다루므로 주소가 필요 없고, 미등재 상대는 주소가 없으면 **판단할 수가 없습니다.** 전부 감추려면 `off`, 전부 보려면 `on`([§6.3.1](design.md)) |
