...
**러너는 한 대가 아닐 수 있습니다.** 망이 갈리면 러너도 갈립니다 — 본사와 DR 센터가
서로 닿지 않으면 각각에 하나씩 놓습니다. 조직 하나에 러너 여럿이 기본 전제이고,
**토큰은 러너마다 따로 발급합니다**([§6.4.1](design.md)) — 한 대가 털렸을 때 그 토큰만
폐기하고 나머지는 계속 돌아야 하기 때문입니다.
...
**① 토큰을 발급받습니다.** 조직 단위이고, 러너가 여러 대면 여러 개 발급합니다 — 하나를
폐기해도 나머지가 살아 있어야 하기 때문입니다([§6.4.1](design.md)).
...
flowchart TB
    subgraph 본사["고객망 · 본사"]
        R1["러너 A<br/>· 본사 대상 목록<br/>· 주소 · 계정 · 키"]
        N1["본사 노드들"]
        R1 -->|SSH| N1
    end
    subgraph DR["고객망 · DR 센터"]
        R2["러너 B<br/>· DR 대상 목록"]
        N2["DR 노드들"]
        R2 -->|SSH| N2
    end
...
| 고객이 여기서 정하는 것 | |
|---|---|
| **토큰** | **러너마다 하나씩** 발급받아 넣습니다. 조직은 토큰에서 유도되므로 러너가 자기 조직을 주장할 수 없고, 한 대만 끊고 싶을 때 그 토큰만 폐기합니다 |
...
**등재는 「어느 러너가 그 노드에 닿는가」도 함께 정합니다.** 러너가 여럿이면 이것이 있어야
작업을 닿는 러너에게만 줄 수 있습니다 — 등재는 러너가 붙어 확인하는 것이라 그 사실이 그
순간 생깁니다. **저장할 자리가 아직 없습니다**(M3).
...
> **지금은 작업이 그 조직의 아무 러너에게나 나갑니다.** 러너↔노드 관계가 아직 없어
> (2단계), DR 노드를 대상으로 한 작업을 본사 러너가 가져갈 수 있습니다. 러너가 한 대인
> 동안은 드러나지 않지만, 여러 대를 전제한 설계이므로 M3에서 닫습니다.
