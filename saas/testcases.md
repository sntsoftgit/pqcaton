...
| **M6** 화면 | orphan 후보가 **근거와 함께** 나옴 · 확신도가 낮은 이유가 표시됨 · 사람이 확정하기 전에는 등재되지 않음 · 지금 주소 노출 모드가 화면에 보임 |
| **M4+** 엣지 해소 | 다른 영역의 상대가 **인증서 지문**으로 이어 붙음(주소 없이도) · 주소 토큰으로도 이어 붙음 · 인증서를 여럿이 나눠 쓰거나 사설 대역이 겹칠 때 **자동 확정하지 않고 후보로만** 올림 |
