...
//
// 왜 게이트인가: pqcaton은 같은 코드를 AGPL-3.0과 상업 라이선스 양쪽으로 낸다.
// 상업 라이선스로 줄 수 있는 것은 그렇게 라이선스할 수 있는 코드뿐이라, 카피레프트
// 라이브러리를 하나라도 링크하면 전체를 비공개 조건으로 낼 수 없다. 듀얼 라이선스가
// 그 자리에서 무너진다.
//
...
// 금지 — 카피레프트. 링크하면 파생물도 같은 조건이 되어 상업 라이선스가 불가능해진다.
...
var forbidden = map[string]string{
	...
	"AGPL-3.0": "카피레프트 — 우리가 AGPL로 내는 것과 별개로, 상업 라이선스가 막힌다",
...
func main() {
	...
	if len(bad) > 0 {
		fmt.Fprintln(os.Stderr, "✗ 링크할 수 없는 라이선스다 — 듀얼 라이선스가 성립하지 않는다:")
		for _, s := range bad {
