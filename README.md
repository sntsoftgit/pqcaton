...
하고, 리눅스와 Windows 를 다룹니다(상류 v0.7.2).
...
**맞는 조합.** 이 리포는 pqcota **v0.7.2**의 계약을 소비하고, 위 명령도 그 릴리스 기준입니다. 계약이 additive라 그보다 낮은 상류에서도 계획은 읽히지만, **승인 검증이 기본으로 닫힌 것과 종료 상태 3은 v0.7.0부터**이고 컨트롤러 CLI가 릴리스에 붙는 것은 **v0.7.2부터**입니다. 그 아래를 쓰면 위 절차가 그대로 맞지 않습니다.
